...
GO
/****** Object:  StoredProcedure [dbo].[sp_PressGaney_UC0103_UrgentCare]    Script Date: 10/21/2025 1:33:54 PM ******/
SET ANSI_NULLS ON
GO
...
------ Description:	Extract dataset for Press Ganey's New File Format for Emergency Department Surveys
------ 
------ SPECIFICATIONS:
------ • Survey Type: Emergency Department - Adult (ER0101) and Pediatric (PE0101)
------ • Data Source: Hospital encounters with Emergency Medicine specialty
------ • File Format: Press Ganey New File Format (NFF) per ITTI specification
------ • Service Area: MaineHealth (ServiceAreaEpicId = '110')
------ 
------ FEATURES IMPLEMENTED:
------ • Encounter-based data extraction for all Emergency Department services
------ • Age-based survey designation (Adult ER0101 vs Pediatric PE0101)
...
------ =============================================*/
ALTER PROCEDURE [dbo].[sp_PressGaney_UC0103_UrgentCare] 
...
-- We will use a CTE to get the patient encounters and then join with other tables as needed
        ...
        ;WITH PatientEncounters AS (
			--{Encounter Type = (Hospital Encounter) AND Specialty = (Emergency Medicine)} AND DEP item 24704 = 2
					SELECT DISTINCT
					...
					WHERE en.DateKey BETWEEN @StartDateInt AND @EndDateInt
						AND en.[Type] IN ('Hospital Encounter')
						AND dep.DepartmentSpecialty IN ('Emergency Medicine')
...
--Select * from PatientEncounters 
	...
	SELECT DISTINCT
		[Survey Designator] = CASE WHEN inpat.AgeInYears >= 18 THEN 'UC0103' ELSE 'UC0104' END,
