...
GO
/****** Object:  StoredProcedure [dbo].[sp_PressGaney_ON0101_Oncology_Department]    Script Date: 10/21/2025 1:33:54 PM ******/
SET ANSI_NULLS ON
GO
...
------ Description:	Extract dataset for Press Ganey's New File Format for Oncology Department Surveys
------ 
------ SPECIFICATIONS:
------ • Survey Type: Oncology Department - Adult (ON0101)
------ • Data Source: Office visits and clinical encounters with Oncology specialties
------ • File Format: Press Ganey New File Format (NFF) per ITTI specification
------ • Service Area: MaineHealth (ServiceAreaEpicId = '110')
------ 
------ ELIGIBLE SPECIALTIES:
------ • Oncology, Gynecologic Oncology, Thoracic Diseases
------ • Hematology and Oncology, Surgical Oncology
------ 
------ ENCOUNTER TYPES INCLUDED:
------ • Office Visit, Clinical Support, Behavioral Health
------ • Nurse Triage, Results Follow-Up, Telemedicine
------ • IMAT, Hospital Encounter
------ 
------ FEATURES IMPLEMENTED:
------ • Encounter-based data extraction for all Oncology Department services
------ • Adult patients only (18+ years) with completed encounters
...
------ • Adult patients only (AgeInYears >= 18)
...
------ =============================================*/
CREATE PROCEDURE [dbo].[sp_PressGaney_ON0101_Oncology_Department] 
...
-- We will use a CTE to get the patient encounters and then join with other tables as needed
		...
		;WITH PatientEncounters AS (
			--{Specialty = Oncology OR Gynecologic Oncology OR Thoracic Diseases OR Hematology and Oncology OR Surgical Oncology.  Encounter Types = Office Visit, Clinical Support, Behavioral Health, Nurse Triage, Results Follow-Up, Telemedicine, IMAT or Hospital Encounter} 
			SELECT DISTINCT
			...
			WHERE en.DateKey BETWEEN @StartDateInt AND @EndDateInt
				...
				AND dep.DepartmentSpecialty IN ('Oncology', 'Gynecologic Oncology', 'Thoracic Diseases', 'Hematology and Oncology', 'Surgical Oncology')
