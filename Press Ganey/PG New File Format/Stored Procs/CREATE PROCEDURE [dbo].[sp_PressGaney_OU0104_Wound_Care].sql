...
GO
/****** Object:  StoredProcedure [dbo].[sp_PressGaney_OU0104_Wound_Care]    Script Date: 10/21/2025 1:33:54 PM ******/
SET ANSI_NULLS ON
GO
...
------ Create date: 10/26/2025
------ Description:	Extract dataset for Press Ganey's New File Format (NFF)
------ Survey Type: Wound Care - Adult Outpatient - OU0104 
------
------ PURPOSE:
------ This stored procedure extracts encounter-based patient data for Wound Care
------ specialty visits and formats it according to Press Ganey NFF specifications.
------
------ SCOPE & FILTERS:
------ - Service Area: 110 
------ - Encounter Types: Office Visit, Clinical Support, Behavioral Health, 
------   Nurse Triage, Results Follow-Up, Telemedicine, IMAT
------ - Patient Age: 18+ years
------ - Specialty: Wound Care
------ - Status: Complete encounters only
------ - Excludes: Deceased patients
------
------ KEY FEATURES:
------ - Demographic data with PG-compliant race/ethnicity mappings
------ - Mobile phone numbers from CLARITY
------ - Up to 6 ICD-10 diagnosis codes per encounter
------ - Provider and department information
------ - Language code mapping for survey delivery
------ - Duplicate prevention via tracking table
------
------ DATA FLOW:
------ 1. Extracts encounters within date range
------ 2. Joins patient demographics and provider details
------ 3. Applies PG formatting rules and mappings
------ 4. Excludes previously sent records
------ 5. Inserts new records to tracking and daily file tables
------
------ ERROR HANDLING:
------ Comprehensive try/catch with error logging via sp_logErrorInfo
------
------ MAINTENANCE NOTES:
------ - Update survey designator if file type changes
------ - Monitor tracking table for data volume
------ - Verify language code mappings periodically
------
------ Last Modified: 10/21/2025
------ Contact: Johnny Croyle for questions or updates
------ =============================================*/

CREATE PROCEDURE [dbo].[sp_PressGaney_OU0104_Wound_Care] 
...
-- We will use a CTE to get the patient encounters and then join with other tables as needed
        ...
        ;WITH PatientEncounters AS (
            --{Specialty = Wound Care. Encounter types = Office Visit,Clinical Support, Behavioral Health, Nurse Triage, Results Follow-Up, Telemedicine, IMAT} 
                    SELECT DISTINCT
                    ...
                    WHERE en.DateKey BETWEEN @StartDateInt AND @EndDateInt
                        ...
                        AND dep.DepartmentSpecialty IN ('Wound Care')
...
--Select * from PatientEncounters 
    ...
    SELECT DISTINCT
        ...
        [Language] = ISNULL(PG_Lang_Code.PG_Code,'0'),
        --[Language] = '99',
