...
GO
/****** Object:  StoredProcedure [dbo].[sp_PressGaney_OU0103_Sleep_Medicine]    Script Date: 10/21/2025 1:33:54 PM ******/
SET ANSI_NULLS ON
GO
...
------ Create date: 10/25/2025
------ Description:	Extract dataset for Press Ganey's New File Format for Wound Care Surveys
------ 
------ SPECIFICATIONS:
------ • Survey Type: Wound Care - Adult Outpatient (OU0104)
------ • Data Source: Hospital encounters and appointments for wound care specialty
------ • File Format: Press Ganey New File Format (NFF) per ITTI specification
------ • Service Area: MaineHealth (ServiceAreaEpicId = '110')
------ 
------ FEATURES IMPLEMENTED:
------ • Encounter-based data extraction for wound care specialty visits
------ • Multi-encounter type support: Office Visit, Clinical Support, Behavioral Health, Nurse Triage, Results Follow-Up, Telemedicine, IMAT
...
------ • Primary and up to 5 additional diagnosis codes (ICD-10-CM format)
...
------ • Complete encounters only (DerivedEncounterStatus = 'Complete')
...
CREATE PROCEDURE [dbo].[sp_PressGaney_OU0103_Sleep_Medicine] 
...
-- We will use a CTE to get the patient encounters and then join with other tables as needed
        ...
        ;WITH PatientEncounters AS (
			--{Speciality = Sleep Center  Encounter types = Office Visit,Clinical Support, Behavioral Health, Nurse Triage, Results Follow-Up, Telemedicine, IMAT} 
					SELECT DISTINCT
					...
					WHERE en.DateKey BETWEEN @StartDateInt AND @EndDateInt
						...
						AND dep.DepartmentSpecialty = 'Sleep Center'
